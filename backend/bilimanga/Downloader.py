...
class Downloader(object):
    def __init__(
        ...
        self.comic_path = os.path.join(
            self.epub_path, check_chars(self.book_name) + "_" + str(self.volume_no)
        )
    ...
    def get_chap_image(
        self, chap_no, chap_name, is_gui=False, signal=None, is_color_page=False
    ):
        ...
        if chap_no == 0:
            self.get_single_image(
                img_url_list[0], self.comic_path, "cover.avif"
            )  # 默认第一张为彩页
        if is_color_page:
            if (
                self.color_page > 1
            ):  # 彩页数量大于1的情况下添加彩页， 否则当没有彩页处理
                ...
                if len(os.listdir(save_path_color)) != len(img_url_list_color):
                    ...
                    for i, img_url in enumerate(img_url_list_color):
                        self.get_single_image(
                            img_url, save_path_color, f"{str(i).zfill(3)}.avif"
                        )
    ...
    def get_single_image(self, img_url, save_path, save_name):
        ...
        try:
            ...
            convert_avif_to_jpg(
                os.path.join(self.temp_path, save_name),
                os.path.join(save_path, save_name.replace(".avif", ".jpg")),
            )
    ...
    def scroll_back(self):
        while True:
            ...
            if unload_bf is None:
                ...
                self.tab.scroll.to_see(
                    self.tab.ele("@@tag()=img@@class=imagecontent lazyload")
                )
            # time.sleep(0.4)
