...
class Editer(object):
    def __init__(
        self, root_path, book_no="0000", volume_no=1, interval=0, num_thread=1
    ):
    ...
    def get_chap_text(self, url, chap_name, return_next_chapter=False):
        ...
        while True:
            ...
            text_chap += text
            url_new = url_ori.replace(".html", "_{}.html".format(page_no + 1))[
                len(self.url_head) :
            ]
    ...
    def get_cover(self, is_gui=False, signal=None):
        ...
        with open(
            os.path.join(self.text_path, "cover.xhtml"), "w+", encoding="utf-8"
        ) as f:
        ...
        for chap_no, url in enumerate(self.volume["chap_urls"]):
            if self.check_url(url):
                if not self.prev_fix_url(chap_no, chap_num):  # 先尝试反向递归修复
                    if (
                        chap_no == 0
                    ):  # 第一个章节都反向修复失败 说明后面章节全部缺失，只能手动输入第一个章节，保证第一个章节一定有效
    ...
    def get_toc(self):
        ...
        with open(
            os.path.join(self.temp_path, "OEBPS/toc.ncx"), "w+", encoding="utf-8"
        ) as f:
            f.write(toc_htmls)
    ...
    def get_content(self):
        ...
        with open(
            os.path.join(self.temp_path, "OEBPS/content.opf"), "w+", encoding="utf-8"
        ) as f:
    ...
    def get_epub_head(self):
        ...
        with open(
            os.path.join(metainf_folder, "container.xml"), "w+", encoding="utf-8"
        ) as f:
            f.write(get_container_html())
        with open(
            os.path.join(self.temp_path, "mimetype"), "w+", encoding="utf-8"
        ) as f:
    ...
    def get_epub(self):
        epub_file = (
            self.epub_path
            + "/"
            + check_chars(self.book_name + "-" + self.volume["volume_name"])
            + ".epub"
        )
